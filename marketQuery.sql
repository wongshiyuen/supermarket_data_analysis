...
\copy supermarket_sales FROM 'supermarket_sales_new.csv' DELIMITER ',' CSV HEADER;
...
\copy (SELECT branch, SUM(unit_price*quantity) AS total_sales, SUM(tax_5percent) AS total_taxes FROM supermarket_sales GROUP BY branch ORDER BY total_sales DESC) TO 'branch_sales.csv' WITH CSV HEADER;

-- QUERY 2: IDENTIFY PRODUCT LINE WITH HIGHEST SALES PER BRANCH
\copy (SELECT branch, product_line, SUM(unit_price * quantity) AS total_sales, SUM(tax_5percent) AS total_taxes FROM supermarket_sales GROUP BY branch, product_line ORDER BY total_sales DESC) TO 'branch_product_sales.csv' WITH CSV HEADER;
